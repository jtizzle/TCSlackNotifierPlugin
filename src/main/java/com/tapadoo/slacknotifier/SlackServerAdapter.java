...
public class SlackServerAdapter extends BuildServerAdapter {
    ...
    private void postStartedBuild(SRunningBuild build) {
        ...
        postToSlack(build, message, "#1dacd6");
    }
    ...
    private void postFailureBuild(SRunningBuild build) {
        ...
        postToSlack(build, message, "danger");
    }
    ...
    private void processSuccessfulBuild(SRunningBuild build) {
        ...
        postToSlack(build, message, "good");
    }
     ...
     */
    private void postToSlack(SRunningBuild build, String message, String color) {
        try {
...
//            payloadObj.addProperty("text", "Build status...");
            ...
            if (build.isHasRelatedIssues()) {
                ...
                issuesAttachment.addProperty("color", color);
            ...
            attachment.addProperty("fallback", "build");
            attachment.addProperty("color", color);

            JsonArray fields = new JsonArray();
            JsonObject field = new JsonObject();

            field.addProperty("title", build.getFullName());
            field.addProperty("value", message);
            field.addProperty("short", true);
